...
function fetchWitness() {
    ...
    const url = '../modules/marriage_license/get-groom-witness.php';
...
$('#witness-form').submit(function(e) {
    ...
    $.ajax({
        url: '../modules/marriage_license/save-groom-witness.php',
        type: 'POST',
        data: formData,
...
function fetchWitnessData(id) {
    ...
    $.get(url, { id }, (response) => {
        console.log(response);
        const rows = JSON.parse(response);
        const { witness_names, residency, name, civil_status, to_marry, id_no, date_issued, issued_at, approved_by } = rows[0];
        ...
        template += `
            <tr>
                <td colspan=2 style="border:none;">
                    I.D. No.: <span class="shared-text" style="width: calc(25%);">${id_no}</span>
                </td>
                <td colspan=2 style="border:none;">
                    <center>
        ...
        template += `
            <tr>
                <td colspan=4 style="border: none;">
                    Date Issued: <span class="shared-text" style="width: calc(25%);">${date_issued}</span>
                </td>
            </tr>
        `;
        ...
        template += `
            <tr>
                <td colspan=4 style="border: none;">
                    Issued at: <span class="shared-text" style="width: calc(25%);">${issued_at}</span>
                </td>
            </tr>
        `;
