<?php
	...
	try {
		...
		$stmt1 = $connection->prepare("INSERT INTO request_type 
										SET `document_type` = 'Birth Certificate', 
											`date_requested` = NOW(), 
											`requested_by_id` = ?, 
											`requested_by` = ?, 
											`copies` = ?,
		...
		$stmt2 = $connection->prepare("INSERT INTO birth 
		...
		$stmt2->bind_param("sssssssssssssssssss", $type, $number_of_copies, $brn, $sex, $lname, $fname, 
		...
		$stmt3 = $connection->prepare("INSERT INTO logs (`user_id`, `logs_detail`, date_created) 
			VALUES (?, CONCAT('Requested Birth Certificate # ', LAST_INSERT_ID()), NOW())");
