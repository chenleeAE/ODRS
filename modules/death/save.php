<?php
	...
	try {
		...
		$stmt1 = $connection->prepare("INSERT INTO request_type 
										SET `document_type` = 'Death Certificate', 
											`date_requested` = NOW(), 
											`requested_by_id` = ?, 
											`requested_by` = ?, 
											`copies` = ?,
		...
		$stmt2 = $connection->prepare("INSERT INTO death 
		...
		$stmt2->bind_param("sssssssssssssss", $type, $number_of_copies, $brn, $sex, $lname, $fname, $mname, $dob, 
			$pob_province, $pob_city, $pob_country, $purpose, $specify, $valid_id_path, $authorization_letter_path);

		$stmt3 = $connection->prepare("INSERT INTO logs (`user_id`, `logs_detail`, date_created) 
			VALUES (?, CONCAT('Requested Death Certificate # ', LAST_INSERT_ID()), NOW())");
