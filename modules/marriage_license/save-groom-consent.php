<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ($id) {
			...
			$sql = "UPDATE consents 
					SET license_id = ?, groom_id = ?, parent_name = ?, parent_address = ?, relationship = ?, 
			...
			if ($stmt = $connection->prepare($sql)) {
				...
				$stmt->bind_param("iissssssssssi", $license_id, $groom_id, $parent_name, $parent_address, $relationship, 
								$child_name, $child_address, $child_age, $to_marry, $to_marry_address, 
			...
			if ($stmt = $connection->prepare($sql)) {
				// Bind the parameters to the prepared statement
				$stmt->bind_param("iissssssssss", $license_id, $groom_id, $parent_name, $parent_address, $relationship, 
								$child_name, $child_address, $child_age, $to_marry, $to_marry_address, 
