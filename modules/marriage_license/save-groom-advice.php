<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ($id) {
			...
			$sql = "UPDATE advice 
					SET license_id = ?, groom_id = ?, sex = ?, place = ?, `date` = ?, advice_to = ?, to_marry = ?, 
			...
			if ($stmt = $connection->prepare($sql)) {
				...
				$stmt->bind_param("iisssssssi", $license_id, $groom_id, $sex, $place, $date, $advice_to, $to_marry, $session_id, $session_name, $id);
			...
			if ($stmt = $connection->prepare($sql)) {
				// Bind the parameters to the prepared statement
				$stmt->bind_param("iisssssss", $license_id, $groom_id, $sex, $place, $date, $advice_to, $to_marry, $session_id, $session_name);
